...
class Button(Widget):
    ...
    def refresh(self):
        ...
        if suffix != self.suffix:
            ...
            self.size = (max(self.size[0], left.get_width() + middle.get_width() + right.get_width()), left.get_height())
...
class Window(Widget, Container):
    ...
    def __init__(self,  position = None , size = (140,50), parent = None, style = None, text = "Window", closeable = True, shadeable = True, moveable = True):
